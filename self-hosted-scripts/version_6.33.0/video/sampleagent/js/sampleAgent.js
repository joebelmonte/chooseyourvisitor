...
document.addEventListener(
  'DOMContentLoaded',
  async function GLANCEvideoDOMLoaded() {
    ...
    // taken from Glance.Lib
    let addScriptAsync = async function (params) {
        return new Promise((resolve, reject) => {
            ...
            for (const attr in params) {
                script.setAttribute(attr, decodeURIComponent(params[attr]))
            }
    ...
    let scriptserver = localStorage.getItem("scriptserver") || "devglancecdn.s3.amazonaws.com";
    let scriptver = localStorage.getItem("scriptver") || '6.5.0M';
    let localserver = localStorage.getItem("localserver");
    let loadBalancer = localStorage.getItem("loadbalancer");
    if(scriptserver == "none") {
      document.getElementById("local_environment_container").style.display = "table-cell";
    }

    if(scriptserver) {
      document.getElementById('environment').value = scriptserver
    }
    if(scriptver) {
      document.getElementById('scriptver').value = scriptver
    }
    if(localserver) {
      document.getElementById('local_environment').value = localserver
    }

    document.getElementById('is-lb').checked = loadBalancer == 'true';

    document.getElementById("load").addEventListener('click', (e) => {
        e.preventDefault()
        localStorage.setItem("scriptserver", document.getElementById('environment').value)
        localStorage.setItem("localserver", document.getElementById("local_environment").value)
        localStorage.setItem("scriptver", document.getElementById('scriptver').value)
        localStorage.setItem("loadbalancer", document.getElementById('is-lb').checked)
    ...
    if(scriptserver == "none") {
      scriptserver = localserver;
    }
    // add scripts pointing to browsercap and videosource
    await addScriptAsync({ src: "https://" + encodeURIComponent(scriptserver) + "/video/source/js/browserCap_" + encodeURIComponent(scriptver) + ".js", type: "text/javascript"})
    await addScriptAsync({ src: "https://" + encodeURIComponent(scriptserver) + "/video/source/js/GlanceVideoSource_" + encodeURIComponent(scriptver) + ".js", type: "text/javascript"})
    ...
    /* get the blur option 'true' or false */
    ...
    const start = async function start( videoElement ) {
      ...
      if(!bgtype) {
        ...
        if(bgtype == "image") {
          ...
          options.bgImageURL = "https://" + scriptserver + "/video/sampleagent/images/video-bgd-1280x720.png";
        } else if(bgtype == "fill") {
          options.bgType = bgtype;
    ...
    const setPlayerlink = ( link, options ) => {
      if( link && options && options.descriptor && options.descriptor.url ) {
        ...
        link.href = `${document.location.origin}/videoplayer/VideoPlayer.aspx?offer=${options.descriptor.url}${params}`

        let versionParams = "&version=" + scriptver;
    ...
    imageFillRadioButton.addEventListener('change', () => {
      if (imageFillRadioButton.value) {
        ...
        let urlFromField = imageURLField.value
        let url = urlFromField ? urlFromField : '../images/bofa-video-bgd-01_1280x716.png'
        sender.setBackgroundSettings('image', null, url)
        sender.setBgFilterActive(true)
      }
    });
*/
    ...
    function replaceBgType(newtype) {
      let urlArray = window.location.href.split("bgtype=");
      let result = "";
      if(urlArray.length > 1) {
        let index = urlArray[1].indexOf("&") >= 0 ? urlArray[1].indexOf("&") : urlArray[1].length;
        if(newtype && newtype.length > 0) {
          result = urlArray[0] + "bgtype=" + newtype + urlArray[1].substring(index);
        } else if(!newtype || newtype.length == 0) {
          result = urlArray[0] + urlArray[1].substring(index);
        }
      } else {
        if(newtype && newtype.length > 0) {
          if(urlArray[0].indexOf("?") < 0) {
            result = urlArray[0] + "?bgtype=" + newtype;
          } else if(urlArray[0].endsWith("?") || urlArray[0].endsWith("&")) {
            result = urlArray[0] + "bgtype=" + newtype;
          } else {
            result = urlArray[0] + "&bgtype=" + newtype;
          }
