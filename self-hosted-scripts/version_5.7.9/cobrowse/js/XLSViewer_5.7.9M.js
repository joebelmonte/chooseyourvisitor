...
window.XLSViewer = class {
  constructor(a) {
    ...
    5 < a.data.length / 1048576 ? (document.querySelector("#xls-toobig").style.display = "block", document.querySelector("#xls-loading").style.display = "none") : (this.workbook = XLSX.read(a.data, {type:"binary", cellStyles:!0}), this.currentIndex = 0, this.total = this.workbook.SheetNames.length, this.HTML = {}, this.HTML[this.currentIndex] = this.getHTML(), this.render(), document.querySelector("#tabtotal").innerHTML = this.total, a.options && a.options.hideDownload ? document.querySelector("#xlslink").style.display = 
  ...
  render() {
    document.querySelector("#the-page").innerHTML = '<div id="table-wrapper">' + this.HTML[this.currentIndex].content + "</div>";
    document.querySelector("#xls-loading").style.display = "none";
    document.querySelector("#pageselect").innerHTML = this.currentIndex + 1;
    document.querySelector("#tabname").innerHTML = this.HTML[this.currentIndex].title;
  }
};
}).call(window);
