 ...
 */
  ...
  async setSource( src ) {
    ...
    /* no change to source */
    if( this.source && this.source.deviceId === source.deviceId ) return
  ...
  * unpause the video stream by restoring the previous preview, that will fire everything up again
  ...
  getMediaStream( params ) {
    ...
    if( existingStream && !this.options.screenshare ) {
      ...
      // whether or not BG effects are one, we need to discard the stream if we switched cameras!
      if (existingStream.active && this.bgBlurEnabled && this.BGEffects && this.BGEffects.effectRenderedStream &&
        ( lastConstraints.deviceId && lastConstraints.deviceId.exact && params.constraints.video.deviceId && params.constraints.video.deviceId.exact ) &&
        ( lastConstraints.deviceId.exact == params.constraints.video.deviceId.exact ) ) {
        discard = false
      }
