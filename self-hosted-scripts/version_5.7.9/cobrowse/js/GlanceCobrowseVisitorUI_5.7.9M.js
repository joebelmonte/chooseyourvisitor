...
class CustomSelect extends HTMLElement {
  connectedCallback() {
    this.innerHTML = '<ul id="glance_select-list" class="glance_select-ul"></ul><div class="glance_select-downarrow"></div><button type="button" role="button" class="glance_select-button glance_guest_controls" aria-haspopup="true" aria-expanded="false" aria-label="Select Camera to use"></button>';
...
;var SSN_COOKIE_NAME = "glance_ssn_info", GLANCEVERSION3 = ["5", "7", "9", "0"].slice(0, 3).join("."), GLANCESCRIPTVERSION = GLANCEVERSION3 + "" + (GLANCE.MINIMIZED ? "M" : "");
...
CBScriptTag.prototype.addStylesheets = function(a) {
  function b(e, f, h) {
    if (!document.getElementById(e)) {
      ...
      var g = document.createElement("link"), k = document.getElementsByTagName("head")[0];
      g.id = e;
      g.type = "text/css";
      g.rel = "stylesheet";
      g.media = "all";
      g.href = f;
      h = h || k.firstElementChild;
      k.insertBefore(g, h);
    }
  }
  function d(e) {
...
function getVisitorLanguage(a, b, d) {
  ...
  for (f = 0; f < e.length; f++) {
    if (b = a.filter(h => h.substr(0, 2).toLowerCase() === e[f].substr(0, 2).toLowerCase()), 0 < b.length) {
      return b[0];
    }
  }
  return d;
...
function getElement(a) {
  function b(c) {
    ...
    this.addEvent = function(e, f, h, g) {
      c.addEventListener(e, function(k) {
        f(k);
    ...
    this.removeAttrs = function(e) {
      let f = c.attributes, h = [];
      for (var g = 0; g < f.length; g++) {
        f[g].name.startsWith(e) && h.push(f[g].name);
      }
      for (g = 0; g < h.length; g++) {
        c.removeAttribute(h[g]);
      }
    };
    ...
    this.makeDialog = function(e, f, h, g, k = !0) {
      function l() {
        f();
        k && m.previousFocus && m.previousFocus.focus();
      }
      var m = this;
      this.trapFocus(!0);
      h && g && getElement(h).addEvent("click", g, !1);
    ...
    this.handleKey = function(e, f, h) {
      this.addEvent("keydown", function(g) {
        if (9 !== e || g.target === g.currentTarget) {
          var k = g.shiftKey || g.ctrlKey || g.altKey;
          g.keyCode === e && (f && g[f + "Key"] || !f && !k) && (h(), g.preventDefault(), g.stopPropagation());
        }
      }, !1);
    };
  }
...
var UIState = {EXPANDED:"expanded", BOXSTATE:"boxstate", RCENABLED:"rcenabled", AGENTJOINED:"agentjoined", DOCSHARETERMSACCEPTED:"docsharetermsaccepted", set:function(a, b) {
...
Confirmation.prototype.show = function(a, b, d, c, e = "", f = visitorText["button-yes"], h = visitorText["button-no"]) {
  ...
  getElement("#glance_confirm_secondary_button").text(h);
...
class VideoPreview extends HTMLElement {
  ...
  showVideoSources() {
    1 < this.videosources.elem.size() && ("function" !== typeof GLANCE.Cobrowse.Visitor.isMobileDevice || !GLANCE.Cobrowse.Visitor.isMobileDevice()) && this.videosources.show();
  }
  handleCameraError(a) {
...
class docShareTerms {
  ...
  showDialog(a) {
    ...
    this.onPreviewConfirm = a.onreviewconfirm;
...
MessageBoxes.prototype.configure = function(a) {
  ...
  if ("original" === this.widgetstyle) {
    ...
    a = {"#glance_msg_ok":"button-message-ok", "#glance_confirm_primary_button":"button-yes", "#glance_confirm_secondary_button":"button-no", "#docshare-loading-header":"docshare-loading-header", "#docshare-disclaimer":"docshare-disclaimer", "#glance_docshare_terms_link":"terms-link", "#glance_file_terms_accept":"button-accept", "#glance_file_terms_decline":"button-decline", "#docshare-header":"docshare-disclaimer-header", "#glance_dragdrop_msg":"docshare-drag-instructions", "#docshare-or":"docshare-or", 
    "#glance_launch_filepicker":"docshare-filepicker", "#glance_invalid_file":"docshare-invalid-file", "#glance_file_too_large":"docshare-file-too-large", "#glance_file_confirm":"docshare-accept", "#glance_file_cancel":"docshare-cancel", "#glance_downloading_msg":"docshare-loading"};
    for (const b in a) {
...
function createSessionButton(a) {
  ...
  if (!getElement("#glance_cobrowse_btn")) {
    ...
    document.addEventListener("keydown", function(c) {
      27 === c.keyCode && document.activeElement.matches("#glance_stop_btn, #glance_resize_video, #glance_stop_video, #glance_blurbgd_video") && (c.target.hasAttribute("data-accessibility-hidden") ? c.target.removeAttribute("data-accessibility-hidden") : c.target.setAttribute("data-accessibility-hidden", ""), c.preventDefault(), c.stopPropagation());
...
function initAPI(a) {
  GLANCE.Cobrowse.VisitorUI.VERSION = "5.7.9.0";
