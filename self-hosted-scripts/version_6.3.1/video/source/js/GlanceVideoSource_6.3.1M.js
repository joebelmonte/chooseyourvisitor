  ...
  * unpause the video stream by restoring the previous preview, that will fire everything up again
  ...
  async getMediaRecorder( params ) {
    ...
    *  see this. https://bugs.chromium.org/p/chromium/issues/detail?id=980822 */
    ...
    // GD-18865 Screenshare support for Firefox
    // resize is a custom event we introduced in polyfilled media player
    mediaRecorder.addEventListener("resize", () => { 
      this.restartAfterOrientationChange(true);
    })
    ...
    return new Promise( ( resolve, reject ) => {
      ...
      playRequestComplete = ( source ) => {
        ...
        if( element.isPlayRequestInProgress ) {
          ...
          // GD-19228 we need the canvas element for safari 16
          if(GLANCE.Video.GlanceVideoSource.getBrowser() === 'Safari' && this.BGEffects && this.BGEffects.effectRenderedCanvas) {
            stream.canvas = this.BGEffects.effectRenderedCanvas;
          }
      ...
      ws.transmitter = ( event ) => {
        ...
        if( this.ws && ws && ws.readyState === 1 && payload ) {
          if (/*!this.options.screenshare && */ ws.bufferedAmount > 0){ 
            ...
            let bufferLimitToReconnect = bytesPerSecond * (this.websocketBufferLimitInSeconds / 1000)
            // the coefficient 0.25 and limit 1000 were figured out by testing on Chrome with throttleing on
            let bufferLimitToDowngrade = 0.25 * bufferLimitToReconnect < 1000 ? 1000 : 0.25 * bufferLimitToReconnect;
            if (ws.bufferedAmount > bufferLimitToDowngrade){ 

              GLANCE.Video.VideoDebug && console.log(this.videoLogFormatter("ws.bufferedAmount exceeds limit of " + bufferLimitToDowngrade + " enough to downgrade"))
              if(!this.profileManager.shouldDowngrade && !this.profileManager.lastDowngradeTime || (now - this.profileManager.lastDowngradeTime > this.profileManager.minDowngradeInterval)) {
                this.profileManager.offererNetworkFailure();
              }
              try {
                // restart the ws after each 5 network related downgrades in a row
                // because ws bufferamount stays high for some reason
                if(ws.bufferedAmount > bufferLimitToReconnect) { 
                  // If there is a big enough overload it's better to drop ws buffer to avoid lag
                  GLANCE.Video.VideoDebug && console.log(this.videoLogFormatter("ws.bufferedAmount exceeds limit of " + bufferLimitToReconnect + " enough to drop the connection"))
    ...
    switch ( cmd ) {
      ...
      case 'Start':
        ...
        if (typeof payload === 'string') {
          ...
          }else if(!this.profileManager.acceptors || Object.keys(this.profileManager.acceptors).length == 0) {
			  // Actually it will never enter this case. No acceptors to calculate dynamic profile constraints 
...
GLANCE.Video.GlanceVideoProfileManager = class {
  constructor(options, bgBlurEnabled, listener){
    ...
    this.qualityNetworkDowngradeResolutionInPercentage = 20
    this.qualityNetworkDowngradeFramerateInPercentage = 20
    this.qualityDeviceDowngradeResolutionInPercentage = 25
    this.qualityDeviceDowngradeFramerateInPercentage = 25
    this.qualityUpgradeResolutionInPercentage = 15
    this.qualityUpgradeFramerateInPercentage = 15
    ...
    this.minDowngradeInterval = 1000
 ...
 //     }
  ...
  addAcceptor(id, clientWidth, clientHeight, screenWidth, screenHeight, devicePixelRatio){
    // When network is really slow we receive Start command that does not contain devicePixelRatio and it causes a lot of confusion
    if(typeof devicePixelRatio == "undefined"
      && this.acceptors[id].devicePixelRatio) {
      devicePixelRatio = this.acceptors[id].devicePixelRatio;
    }
    ...
    if (this.state == GlanceVideoProfileManagerState.UNINITIALIZED){
      ...
      this.acceptors[id] = {width: clientWidth, height: clientHeight, devicePixelRatio: devicePixelRatio}
      ...
      let maxHeight = maxRes.height > this.maximumHeight ? this.maximumHeight : maxRes.height
    ...
    // Update profile only if it's the first acceptor or there was a significant change in player's video tag sizes (resize, maximize/minimize)
    if (!this.width || !this.height || newMin && oldMin && (Math.abs(oldMin.height - newMin.height) > 10)){
        GLANCE.Video.VideoDebug && console.log("Update Profile on Add Acceptor " + newWidth + ", " + newHeight + "                       " + oldMin.height + ", " + newMin.height);
        this.reset();
  ...
  updateProfile(newWidth, newHeight, newFramerate){
    ...
    if (this.width != oldWidth || this.height != oldHeight || this.framerate != oldFramerate){
      this.clearFrameTimestamps()
      this.notifyListener()
      // Logging for testing and debugging purposes
      if(this.height > oldHeight || this.framerate > oldFramerate) {
        GLANCE.Video.VideoDebug && console.log("Upgrade : " + this.width + ", " + this.height + ", " + this.framerate)
      } else {
        GLANCE.Video.VideoDebug && console.log("Downgrade : " + this.width + ", " + this.height + ", " + this.framerate)
      }
    }
        ...
        try {
          ...
          if (sessionProfileString){
            ...
            if (sessionProfile.width && sessionProfile.height && sessionProfile.framerate){
              GLANCE.Video.VideoDebug && console.log("Video profile manager: Fetched saved profile from session storage: " + sessionProfileString)
  ...
  performUpgrade(){
    ...
    if (this.stabilityLevel > 1){
      ...
      GLANCE.Video.VideoDebug && console.log("Video profile manager: Stability is now " + this.requiredStabilityInSeconds + " seconds (" + this.requiredStabilityIntervals + " successful keyframe intervals).")
    }
  }
  ...
  performDynamicUpgrade(){
    if (this.framerate && this.width && this.height){
      ...
      if(this.newMinimumHeight || (newWidth < this.maximumWidth)) {
  ...
  performDynamicDowngrade(reason){
    if (this.framerate && this.width && this.height){
      ...
      if (this.framerate > this.goalFramerate){
        ...
        if (this.getDowngradeAspect(reason) == "framerate") {
  ...
  // Checks whether we should downsize the resolution or reduce framerate
  getDowngradeAspect(reason) {
    let resolutionPercentage = this.height * 100 / this.maximumHeight;
    let frameratePercentage = this.framerate * 100 / this.maximumFramerate;
    let framerateResolutionRatio = 2;
    if(reason && reason == GlanceVideoProfileManagerDowngradeReason.OFFERER_NETWORK || reason == GlanceVideoProfileManagerDowngradeReason.ACCEPTOR_NETWORK) {
      // If network is slow it's better to downgrade resolution to save more bendwidth
      framerateResolutionRatio = 0.5;
    }

    if(resolutionPercentage > frameratePercentage * framerateResolutionRatio) {
      return "resolution"
    }
    return "framerate";
  }
  ...
  frame(frameIndex, framesPerInterval){
    ...
    if (this.frameTimestamps.length >= this.frameTimestampsToSample) {
      ...
      // To get the avarage value we should devide by this.frameTimestamps.length - 1 and not this.frameTimestamps.length as was before
      let avg = diff / (this.frameTimestamps.length - 1)
      let fps = 1000.0 / avg
    ...
    if ((frameIndex % framesPerInterval) == 0){
      ...
      if ((this.successfulKeyframeIntervals > 0) 
        && ((this.successfulKeyframeIntervals % this.requiredStabilityIntervals) == 0)){
          this.upgradesInaRowCount++;
          if(this.upgradesInaRowCount > 3) {
            this.upgradesInaRowCount = 0;
            // If there are at least three upgrades in a tow hit the maximum and start from there
            // We need this because downgrade speed is much higher than upgrade speed
            // So when network conditions are improved and stabilized we can re-start our way from the top to keep the quality as high as possible
            this.resetToMax();
          } else {
            this.upgrade()
          }
  ...
  resetToMax() {
    let maxRes = this.getAcceptorMaximumResolution()
    this.updateProfile(maxRes.width, maxRes.height, this.maximumFramerate)
  }
  ...
  offererNetworkFailure(){
    GLANCE.Video.VideoDebug && console.log("Video profile manager: Couldn't offer stream because of network issues")
  ...
  offererEncodingFailure(){
    ...
    GLANCE.Video.VideoDebug && console.log("Video profile manager: Couldn't offer stream because frames could not be encoded at framerate")
  ...
  acceptorNetworkFailure(id){
    ...
    GLANCE.Video.VideoDebug && console.log("Video profile manager: Acceptor player " + id + " couldn't render stream because of their network")
    this.downgrade(GlanceVideoProfileManagerDowngradeReason.ACCEPTOR_NETWORK)
  }

  acceptorRenderFailure(id){
    GLANCE.Video.VideoDebug && console.log("Video profile manager: Acceptor player " + id + " couldn't render stream because of their device")
  ...
  legacySlower(){
    ...
    GLANCE.Video.VideoDebug && console.log("Video profile manager: Slower requested without stated reason by vserver or acceptor.")
    this.downgrade(GlanceVideoProfileManagerDowngradeReason.ACCEPTOR_NETWORK)
  }
