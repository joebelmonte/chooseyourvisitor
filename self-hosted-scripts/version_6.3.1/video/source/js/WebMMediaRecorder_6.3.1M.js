  ...
  : (function() {
    ...
    MediaRecorder.prototype = {
      ...
      _getActivePreview: function _getActivePreview( stream ) {
        let result
        // GD-19228 Safari 16 only works if we draw the webgl canvas element into 2d canvas instead of video preview element
        // For that reason we set a .canvas property of stream in GlanceVideoBGEffects. 
        // BTW in case of Firefox the stream object has property .canvas on his own
        if(GLANCE.Video.GlanceVideoSource.getBrowser() === 'Safari' && this.stream.canvas ) {
            result = this.stream.canvas;
        } else if(stream && stream.mostRecentlyActivatedPreview) {
      ...
      _startRecording: function(){
        if (this.workerLoaded){
          ...
          if ((width > 0) && (height > 0)){
            ...
            if (this.widthRoundedUp = ((width % 2) != 0)) {
                width += 1
            }
            if ((height % 2) != 0){
                height += 1
      ...
      captureVideoFrame: function(){
        ...
        let width = GLANCE.Video.GlanceVideoSource.getBrowser() === 'Safari' ? this.actualWidth : videoElement.videoWidth
        let height = GLANCE.Video.GlanceVideoSource.getBrowser() === 'Safari' ? this.actualHeight : videoElement.videoHeight

        if (width == 0 || height == 0){
          return;
        }

        // We decreased odd number to be even for muxer to work 
        // Now we increase it back to avoid Firefox distortions on screenshare
        if(this.widthRoundedUp && width > 0) {
            width = width + 1;
        }

        this._imagePending = true

        if((this.previousHeight != height || this.previousWidth != width) && this.previousHeight != 0) {
